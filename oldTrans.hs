...
import Data.List (nub, group, sort,isPrefixOf, partition, stripPrefix, intercalate)
import Parser
...
main args = do
    let imm      = "-imm" `elem` args
        npm      = "-npm" `elem` args
        filename = head (filter (`notElem` ["-imm", "-npm"]) args)
    ...
    let inputLines  = lines inputFile
        ...
        rules       = (if imm then addAllErasers . addAllDuplicators else id) (expandAllGenConstrs [r | Right r <- parsedRules])
        errors      = [e | Left e <- parsedRules]
    if not (null errors)
       then error ("Parse errors: " ++ show errors)
       else do
           let builtinLUT = [("succ", 1), ("pred", 1), ("!eraser", 0), ("!duplicator", 0)]  -- needs to have special rules in transRuleList or elsewhere for each
...
applyT lhs t2 root lut npm =
    case findNestedAgent lhs of
        Nothing ->
            let rawRhs   = trans t2 root ([], []) lut
                cleanRhs = cleanNet rawRhs
            in trace ("  [Nothing] root: " ++ root) $
               trace ("  [Nothing] t2: " ++ show t2) $
               trace ("  [Nothing] raw rhs: " ++ show rawRhs) $
               trace ("  [Nothing] clean rhs: " ++ show cleanRhs) $
               transActivePair lhs ++ netToINPLA cleanRhs
        Just (alpha, beta, gamma, nestedPort, a) ->
            ...
            in trace ("  [Just] alpha: " ++ show alpha) $
               trace ("  [Just] beta: " ++ show beta) $
               trace ("  [Just] gamma: " ++ show gamma) $
               trace ("  [Just] nestedPort: " ++ nestedPort) $
               trace ("  [Just] abName: " ++ abName) $
               trace ("  [Just] abAux: " ++ show abAux) $
               trace ("  [Just] lhs2: " ++ show (lhs2 :: Net)) $
               trace ("  [Just] root: " ++ root) $
               ruleI ++ "\n" ++ ruleII
...
testFindNestedAgent :: FilePath -> IO ()
testFindNestedAgent filename = do
    inputFile <- readFile filename
    let inputLines  = lines inputFile
        ruleLines   = filter (not . ("--" `isPrefixOf`)) inputLines
        parsedRules = map parseRule ruleLines
        rules       = expandAllGenConstrs [r | Right r <- parsedRules]
        builtinLUT  = [("succ", 1), ("pred", 1), ("!eraser", 0), ("!duplicator", 0)]
        lut         = makeLUT rules builtinLUT
    mapM_ (\r@(Rule t1 _) -> do
        let lhs = cleanNet $ trans t1 "r" ([],[]) lut
        putStrLn $ "Rule: " ++ show r
        putStrLn $ "LHS net: " ++ show lhs
        putStrLn $ "Nested agent: " ++ show (findNestedAgent lhs)
        putStrLn ""
        ) (filter hasNestedPattern rules)
    let t2   = Par (Func "last" [Constr "Cons" [Var "y", Var "ys"]]) (Func "e" [Var "x"])
        lut2 = [("last",1),("e",0)]
    putStrLn $ "raw RHS: " ++ show (trans t2 "r" ([],[]) lut2)
    putStrLn $ "clean RHS: " ++ show (cleanNet $ trans t2 "r" ([],[]) lut2)